...
// [[Rcpp::plugins("cpp11")]]
// [[Rcpp::export]]
vector<int> PPD_SEIR_StandardILM(int lp, int ip, vector<int> exp_time, vector<double> x, vector<double> y, int t_start, int t_end, vector<double> parameters){
  double a0 = parameters[0];
  double beta = parameters[1];
  int N = exp_time.size();//population size
  //copy inf time before t_start
  vector<int> ppd(N,0);
  for(int i=0; i<N; i++){
    if(exp_time[i]<t_start){
      ppd[i] = exp_time[i];
    }
  }
  //simulate ppd
  ...
  for(int t=t_start; t<=t_end; t++){
    for(int i=0; i<N; i++){
      if(ppd[i]==0){
        ...
        //calculate kernel
        for(int j = 0; j<N; j++){
          if(i!=j && ppd[j]!=0 && ppd[j]+lp<t && ppd[j]+lp+ip>=t){
...
// [[Rcpp::plugins("cpp11")]]
// [[Rcpp::export]]
vector<int> PPD_SEIR_M2(int lp, int ip, vector<int> exp_time, vector<double> x, vector<double> y, int t_start, int t_end, vector<int> assign, vector<double> parameters){
  double a0 = parameters[0];
  double beta = parameters[1];
  ...
  for(int t=t_start; t<=t_end; t++){
    ...
    for(int i=0; i<N; i++){
      if(ppd[i]!=0 && ppd[i]+lp<t && ppd[i]+lp+ip>=t){
        inf_num[assign[i]]++;
      }
    }
    for(int i=0; i<N; i++){
