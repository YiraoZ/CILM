...
// [[Rcpp::plugins("cpp11")]]
// [[Rcpp::export]]
double log_likelihood_SEIR_basic(int lp, int ip, vector<int> exp_time, vector<double> x, vector<double> y, int t_start, int t_end, vector<double> parameters){
  ...
  for(int t=t_start; t<=t_end; t++){
    ...
    for(int i=0; i<N; i++){
      if(exp_time[i]==0 || exp_time[i]>=t) {//make sure the individual is susceptible at time t-1
        double kernel = 0;
        //calculate kernel
        for(int j=0; j<N; j++){
          if(i!=j && exp_time[j]!=0 && exp_time[j]+lp<t && exp_time[j]+lp+ip>=t){
...
// [[Rcpp::plugins("cpp11")]]
// [[Rcpp::export]]
double log_likelihood_SEIR_M2(int lp, int ip, vector<int> exp_time, vector<double> x, vector<double> y, int t_start, int t_end, vector<int> assign, vector<double> parameters){
  double a0 = parameters[0];
  double beta = parameters[1];
  ...
  for(int t=t_start; t<=t_end; t++){
    ...
    for(int i=0; i<N; i++){
      if(exp_time[i]!=0 && exp_time[i]+lp<t && exp_time[i]+lp+ip>=t){
        inf_num[assign[i]]++;
      }
    }
    //loop through all individuals
